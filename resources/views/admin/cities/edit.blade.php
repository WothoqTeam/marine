...
<div class="page-title d-flex flex-column justify-content-center gap-1 me-3 pt-6">
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
        ...
        <li class="breadcrumb-item text-muted">
            <a href="{{route('admin.countries.index')}}" class="text-muted text-hover-primary">{{trans('labels.settings.cities_tap')}}</a>
        </li>
        <!--end::Item-->
        <!--begin::Item-->
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="card mb-5 mb-xl-10">
            ...
            <div id="kt_account_settings_profile_details" class="collapse show">
                <!--begin::Form-->
                <form action="{{route('admin.cities.update')}}" method="POST" enctype="multipart/form-data" id="kt_account_profile_details_form" class="form">
                    @csrf
                    ...
                    <div class="card-body border-top p-9">
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8 fv-row">
                                <input type="text" name="name_en" placeholder="{{trans('labels.inputs.name')}} EN" value="{{$data['cities']->name_en}}" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0" />
                            </div>
                        </div>
                        <div class="row mb-6">
                            <label class="col-lg-2 col-form-label required fw-semibold fs-6">{{trans('labels.inputs.name')}} AR</label>
                            <div class="col-lg-8 fv-row">
                                <input type="text" name="name_ar" placeholder="{{trans('labels.inputs.name')}} AR" value="{{$data['cities']->name_ar}}" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0" />
                            </div>
                        </div>
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-8 fv-row">
                                <select name="country_id" aria-label="{{trans('labels.inputs.select')}} {{trans('labels.inputs.country')}}" data-control="select2" data-placeholder="{{trans('labels.inputs.select')}} {{trans('labels.inputs.country')}}..." class="form-select form-select-solid form-select-lg fw-semibold">
                                    ...
                                    @foreach($data['countries'] as $country)
                                        <option value="{{$country->id}}" @if($data['cities']->country_id == $country->id) selected @endif >{{$country->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-0">
                            ...
                            <div class="col-lg-8 d-flex align-items-center">
                                <div class="form-check form-check-solid form-switch form-check-custom fv-row">
                                    <input class="form-check-input w-45px h-30px" type="checkbox" name="status" value="1" id="allowmarketing" @if($data['cities']->status == 1) checked="checked" @endif />
